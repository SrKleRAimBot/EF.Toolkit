...
public static class DbContextBulkExtensions
{
    ...
    /// <summary>
    ///     Saves the change tracker's pending changes through EF.Bulk, without needing
    ...
    ///         every save already goes through EF.Bulk's batch factory, so there is nothing extra to
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        // Resolved purely to fail loudly when EF.Bulk is not configured; without the check this
        // would silently be a plain, unaccelerated save.
        _ = context.GetService<EFBulk.Configuration.BulkOptions>()
            ?? throw new InvalidOperationException(
                "EF.Bulk is not configured for this context. Add UseBulkOperations() alongside "
    ...
    /// <summary>Saves the change tracker's pending changes through EF.Bulk, synchronously.</summary>
    /// <param name="context">The context. Must be configured with <c>UseBulkOperations()</c>.</param>
