...
namespace EFBulk.Infrastructure;
...
public class BulkModificationCommandBatch : ModificationCommandBatch
{
    ...
    ///     front, because a single EF.Bulk batch can fall back to several provider batches.
    /// </param>
    ...
    /// <summary>The context's EF.Bulk settings.</summary>
    ...
    ///     latter, so without this an EF.Bulk-enabled context would break existing error handling
    ...
    private IReadOnlyList<BulkPartition> EnsureCompleted()
        => _partitions
            ?? throw new InvalidOperationException(
                ...
                + "EF.Bulk's integration with EF Core's update pipeline.");
        ...
        foreach (var partition in partitions)
        {
            if (partition.IneligibleReason is { } reason)
            {
                throw new BulkNotSupportedException(
                    $"EF.Bulk cannot accelerate {partition}: {reason} "
    ...
    private void ThrowIfUnsupported(BulkPartition partition, string? reason)
    {
        ...
        {
            throw new BulkNotSupportedException(
                $"EF.Bulk cannot accelerate {partition}: {reason ?? "no reason given."} "
                + "Configure OnUnsupported(Unsupported.FallBack) to run it through EF Core "
                + "instead.");
        }
    }
