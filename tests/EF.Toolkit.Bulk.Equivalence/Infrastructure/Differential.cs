...
/// <summary>
///     Runs a scenario twice — once through stock EF Core, once through EF.Toolkit.Bulk — and asserts the
...
public static class Differential
{
    ...
    ///     tracker state, and failure behaviour under stock EF Core and under EF.Toolkit.Bulk.
    /// </summary>
        ...
        Func<ShopContext, Task> scenario)
    {
        ...
        {
            // The scenario failing is itself a comparable outcome: stock EF and EF.Toolkit.Bulk must fail
    ...
    ///         violates a constraint with an <c>INSERT</c>, EF.Toolkit.Bulk with a bulk copy or a staged
        ...
        string? messagesMayDiffer)
    {
        switch (stock, bulk)
        {
            ...
            case (null, not null):
                return "Scenario succeeded under stock EF but threw under EF.Toolkit.Bulk:"
                    + Environment.NewLine + bulk;

            case (not null, null):
                return "Scenario threw under stock EF but succeeded under EF.Toolkit.Bulk:"
        ...
        for (var depth = 0; stockLink is not null || bulkLink is not null; depth++)
        {
            if (stockLink is null || bulkLink is null)
            {
                return "The two failures wrap different numbers of exceptions: at nesting depth "
                    + $"{depth} stock EF has {Name(stockLink)} and EF.Toolkit.Bulk has {Name(bulkLink)}.";
            }
            ...
            if (stockLink.GetType() != bulkLink.GetType())
            {
                ...
                return $"Stock EF threw {Name(stockLink)} but EF.Toolkit.Bulk threw {Name(bulkLink)}{where}.";
            }
