...
///     bulk execution paths land, EF.Toolkit.Bulk delegates everything to the provider, so the two sides
...
public abstract class HarnessSelfTests(DatabaseFixture fixture)
{
    ...
    public async Task Detects_a_failure_on_only_one_side()
    {
        ...
        failure.ShouldContain("succeeded under stock EF but threw under EF.Toolkit.Bulk");
    }
    ...
    public async Task Detects_the_same_failure_reported_differently()
    {
        var failure = await CaptureFailure(context =>
        {
            ...
            // wrong -- which constraint, which row -- would behave differently under EF.Toolkit.Bulk.
