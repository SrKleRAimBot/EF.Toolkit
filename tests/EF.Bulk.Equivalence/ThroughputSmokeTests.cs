...
/// <summary>
///     A coarse check that EF.Bulk is actually faster than stock EF Core.
/// </summary>
/// <remarks>
///     Not a benchmark — that belongs in <c>EF.Bulk.Benchmarks</c> with proper warmup and
...
public abstract class ThroughputSmokeTests(DatabaseFixture fixture, ITestOutputHelper output)
{
    ...
    public async Task Bulk_insert_is_faster_than_stock_ef()
    {
        ...
        output.WriteLine($"EF.Bulk  : {bulk.TotalMilliseconds,8:F0} ms  "
        ...
        bulk.ShouldBeLessThan(
            stock,
            $"EF.Bulk ({bulk.TotalMilliseconds:F0} ms) was not faster than stock EF "
    ...
    public async Task Explicit_bulk_insert_beats_transparent_mode()
    {
        ...
        output.WriteLine($"stock SaveChanges   : {stock.TotalMilliseconds,8:F0} ms  "
            + $"({Rows / stock.TotalSeconds,9:F0} rows/sec)");
        output.WriteLine($"transparent EF.Bulk : {transparent.TotalMilliseconds,8:F0} ms  "
