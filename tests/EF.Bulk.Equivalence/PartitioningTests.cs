...
/// </summary>
/// <remarks>
///     The equivalence suite proves EF.Bulk produces the same results as stock EF. On its own that
...
public abstract class PartitioningTests(DatabaseFixture fixture)
{
    ...
    public async Task A_batch_containing_several_shapes_is_split_and_still_correct()
    {
        ...
        recorder.Batches.ShouldNotBeEmpty("EF.Bulk planned no batches at all.");
    ...
    public async Task Partitions_report_why_they_did_not_accelerate()
    {
        ...
        // the failure mode that would make EF.Bulk look correct while delivering nothing.
    ...
    public async Task Client_generated_keys_actually_take_the_bulk_path()
    {
        ...
        // Passing the equivalence suite proves EF.Bulk is correct; it does not prove it is fast.
